...
- [x] 🔥 ~~Estendere a `evaluate_tsne` il precompute della matrice di distanze per `metric="euclidean"`~~ — **Fatto 25/08** (stessa sessione, poco dopo il fix UMAP iniziale). `evaluate_tsne`/`reduction.py::embed()` ora usano `PRECOMPUTABLE_METRICS`/`precomputed_distance` esattamente come `evaluate_umap` — vedi `docs/debugging/debug_25_08_26.md` §1 e `.claude/lessons_learned.md` #29. Nota: a differenza di UMAP (88s vs 17-35 min, misurato direttamente), il guadagno di velocità per t-SNE non è stato misurato in modo indipendente — il fix è comunque corretto/sicuro (stesso meccanismo già in produzione per jaccard/dice, solo esteso a una metrica in più), ma l'entità reale dello speedup per t-SNE resta da verificare quando si lancerà davvero un tuning t-SNE su s1.2.
