...
(*) per ogni clustering: analisi/visualizzazione dedicata (mappe su MNI, scatter colorato,
    overlap con atlanti, radar/spider plot del centroide, matrici FC per cluster...)
(**) l'aggregazione/confronto multi-dataset richiede armonizzazione tra scanner (neuroCombat) —
     necessaria almeno per WashU ST vs HC che hanno scanner diversi (Seba_Meeting_3, sezione J)
```
...
| -------------------------------------------------------------------------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. **Embedding lesioni** + **clustering** (n~5750)                                                       | Task 1                | PCA(varimax) / UMAP / t-SNE su lesioni parcellizzate; <br>clustering: k-means, HDBSCAN, agglomerative, GMM, spectral (o altri da letteratura); <br>analisi/viz sul clustering risultante: mappa di probabilità delle lesioni per cluster su MNI, scatter dell'embedding colorato per dataset (per valutarne la copertura reciproca) e per subitem NIHSS specifici — es. item 9=linguaggio, item 11=neglect, 5a-6b=motorio, dato che ogni subitem mappa un dominio comportamentale specifico (`Seba_Meeting_1`) — overlap % con atlanti (vascolare/Yeo/Figley), radar/spider plot del centroide di cluster (metriche, connettività, demografici, score clinici) | **Thiebaut** de Schotten 2020 (lesioni non casuali, seguono la vascolarizzazione → clusterizzano)                                                         | Replicare la PCA varimax di Thiebaut de Schotten (46 componenti, 30 spiegano >90% varianza) sulla nostra N molto più ampia; confrontare i cluster con i territori vascolari noti | Confrontare sistematicamente più combinazioni embedding×clustering alla scala n~5750                                                                                                                                                                                                                        |
| 2. **Embedding SDC** + **clustering**                                                                    | Task 2                | Stessi metodi di riduzione e clustering della fase 1, su SDC parcellizzata (BCB Toolkit); <br>stessa analisi/viz della fase 1 (mappa su MNI, scatter colorato, overlap atlanti, radar plot del centroide)                                                                                                                                                                                                                                              | **Griffis** 2019/2020 (SDC spiega la disfunzione di rete meglio del danno locale); <br>**Salvalaggio** 2020 (SDC ≈ lesione come predittore, FDC fallisce) | Replicare il morfospazio **UMAP** di Talozzi 2023, ma partendo da SDC (non lesione grezza) e sulla nostra coorte                                                                 | —                                                                                                                                                                                                                                                                                                           |
| 2b. **Embedding + clustering FC indipendente** (n~500, WU+PD+Fri)                                        | Task 3                | PCA/UMAP su matrice FC statica (fattibile ora, riusa `dim_reduction`); <br>autoencoder su dinamica BOLD se disponibile; <br>clustering: k-means, HDBSCAN, agglomerative, GMM, spectral (o altri da letteratura); <br>analisi/viz: matrice FC media per cluster e diff vs coorte sana, riassunto per network Yeo (within/between), feature locali (ReHo/ALFF) su superficie, composizione per dataset/sito (controllo artefatto scanner)                | **Idesis** 2023 (autoencoder su dinamica BOLD batte PCA lineare, predice recupero a 1 anno)                                                               |                                                                                                                                                                                  | Dà un clustering FC indipendente da usare come "vero" fenotipo per validare l'inferenza anatomia→funzione (fase 5), in aggiunta al ruolo descrittivo che la FC ha già in fase 4                                                                                                                             |
