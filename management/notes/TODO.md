...
- [x] 💭 ~~Rimuovere del tutto la feature `parcellate` da `build_lesion_matrix.py`~~ — **Deciso 25/08: NON rimuovere, tensione risolta a favore di tenerla.** La motivazione originale (2026-08-17, CRITICAL #6/#7 dell'audit del 15/08 — `enrich_metadata_with_lesion_info` che si rifiutava su matrici parcellate, `"volume"` color mode senza guard di binarietà) è stata **risolta diversamente** durante la stessa sessione del 17-18/08, non da questa rimozione: `enrich_metadata_with_lesion_info` è stata tolta dalle pipeline (`dim_reduction.py`), `build_lesion_matrix()` calcola sempre `lesion_volume_voxels` da `X_voxelwise` prima di un'eventuale parcellazione, e ogni color mode (`embedding_coloring.py`) legge oggi una colonna già persistita in `metadata.csv` invece di ricalcolare dal vivo su `X` — nessuna delle due funzioni dipende più dalla binarietà di `X`. Il conflitto con la produzione `yan300s1`/atlante Yan300TianS2Buckner7N (replica Thiebaut de Schotten 2020) non esiste più: `parcellate` resta nel config/codice. `build_lesion_matrix.json` esteso 25/08 a UCL-UK/UCLStrokeData (`session_name: "yan300s2"`), stesso `parcellate: true`.
