...
- [ ] 💭 **Dimensionalità dell'embedding per il clustering**: decidere n_components (2 vs 5 vs 10). Nessun precedente in letteratura prescrive un default valido in generale (verificato 2026-08 su 4 fonti — Talozzi 2023, de Bodt et al. 2025, doc ufficiale `umap-learn`, critica indipendente — vedi `docs/notes/dim_reduction.md` sezione "UMAP e clustering: quante componenti?"). Passi propedeutici, in ordine di priorità:
  ...
  - [ ] 🔥💭 2. Decidere/implementare come `clustering.py` deve gestire un embedding in input con k>2 componenti — oggi plotta sempre le prime 2 colonne grezze (`X[:, :2]`), corretto solo per matrici raw non ridotte (documentato esplicitamente così in `docs/dev/models.md`), ma un vero slice-invalido (`lessons_learned.md` #16) se in input arriva un embedding UMAP/t-SNE/PaCMAP a k>2. Serve un refit dedicato per la viz (come già fa `dim_reduction_clustering.py` via `embedding_for_viz`) o un rifiuto esplicito quando non è possibile, mai uno slice silenzioso.
  - [ ] 🔥 3. Eliminare `dim_reduction_clustering.py` come pipeline separata (senza cancellare i suoi risultati esistenti, legacy), assicurandosi prima che `clustering.py` copra ogni sua funzionalità: il tuning è già a parità, ma manca almeno il comparison plot interattivo (`plot_clusters_comparison_interactive`) e, a seconda del punto 2, il supporto a `viz_n_components`.
