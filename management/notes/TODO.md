...
- [ ] 💭 **Rimuovere del tutto la feature `parcellate` da `build_lesion_matrix.py`** (2026-08-17, deciso durante la sessione che ha chiuso i bug CRITICAL #6/#7 dell'audit del 15/08 — `enrich_metadata_with_lesion_info` che si rifiutava su matrici parcellate, `"volume"` color mode senza guard di binarietà): togliere `parcellate`/`atlas_path`/`parcel_aggregation`/`save_parcellated_volumes` dal config e dal codice (`src/features/lesion.py`: `_parcellate_matrix`, `load_and_resample_atlas`, `PARCEL_AGGREGATIONS`), non solo ripulire i campi da un singolo file di config. **Nota di tensione da risolvere prima di procedere**: la produzione attuale (`config/pipelines/build_lesion_matrix.json`, sessione `yan300s1`) usa `"parcellate": true` con l'atlante Yan300TianS2Buckner7N proprio per riprodurre la metodologia sotto — rimuovere la feature blocca la riga seguente, non solo ripulisce codice morto. Va deciso esplicitamente se questa direzione sostituisce l'obiettivo di replica, prima di implementare.
