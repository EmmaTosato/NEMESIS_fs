...
- **`k` (il numero di vicini controllati) è un parametro a parte**, `trustworthiness_n_neighbors` in `params_reduction.json` — **non** lo stesso `n_neighbors` che UMAP sweeppa per costruire il proprio grafo. Sono due concetti distinti tenuti deliberatamente separati (`docs/dev/config.md`).
...
- **Tra cosa si può confrontare, quindi**: solo combinazioni allo **stesso** `n_components` — `metric` diverso, `n_neighbors`/`min_dist` diversi, tutti confrontabili tra loro finché `n_components` resta fisso.
...
- `embeddings.npz` (solo se `save_tuning_embeddings: true` in `dim_reduction.json`, opt-in, `false` di default): un unico file in cima alla cartella del run con l'embedding vero e proprio di **ogni** combinazione valutata dallo sweep — non solo quelle disegnate in `embeddings_grid_*.png`. Ogni array è indicizzato da una chiave leggibile `"chiave1=valore1,chiave2=valore2,..."`, stessi nomi/valori delle colonne di `tuning_results.csv` (nell'ordine di `tuning_grid` di quella run) — per recuperare l'embedding di una riga basta ricostruire la stessa chiave dai suoi valori, senza bisogno di un indice separato. Esiste per alimentare un futuro plot interattivo (griglia stile Figura 4 di [`pair-code.github.io/understanding-umap`](https://pair-code.github.io/understanding-umap/), vedi sotto) che deve solo leggere e disegnare, senza rifittare nulla.
- `metadata.csv` (stesso trigger di `embeddings.npz`, un solo file per l'intero run, non per foglia): anagrafica arricchita — stesso schema/stessa funzione (`enrich_metadata_with_lesion_info`) della produzione, con `lesion_volume_voxels`/`lesion_side`/`nihss` già calcolati come colonne letterali. Necessario perché `embeddings_grid_*.png` colora i punti calcolando `side`/`nihss`/`volume` **al volo** (join live contro il registro clinico, somma su `X`) — `embeddings.npz` da solo contiene solo coordinate anonime, senza questo file un plot futuro dovrebbe ricaricare `X` e ricontattare il registro esterno per sapere di chi è ogni punto.
...
## Verso una griglia interattiva stile "Understanding UMAP"

`embeddings.npz` è il prerequisito dati per replicare, sui nostri dati reali (mai su dataset sintetici), la Figura 4 della pagina PAIR sopra citata: una griglia `n_neighbors` × `min_dist` con un plot interattivo al posto della `embeddings_grid_*.png` statica — nessun ricalcolo UMAP lato browser, solo lettura e plotting Plotly degli array già salvati. Non ancora implementato (fase successiva, tracciata separatamente).
