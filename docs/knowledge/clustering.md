...
Non è legato a un metodo di base fisso: prende **uno qualsiasi** tra KMeans/GMM/Spectral (`base_method` in config), lo ripete `n_repeats` volte con semi casuali diversi, e conta quante volte ogni coppia di soggetti finisce nello stesso cluster → matrice di co-occorrenza `C`. Le etichette finali **non** vengono da una singola delle `n_repeats` run, ma derivate da `C` stessa, con clustering gerarchico **single-link** su `1 - C`, tagliato a `n_clusters` — l'algoritmo di Fred & Jain (2002), "Data clustering using evidence accumulation" (`papers/sota/Fred et al - 2002...md`, verificato riga per riga contro lo pseudocodice del paper).

- **RSC (Zanola et al. 2026, `papers/nemesis/Zanola et al - 2026...`) è esattamente questo metodo con `base_method="spectral"`** — non un metodo a parte. Il paper di Zanola non specifica l'algoritmo esatto per l'ultimo passo ("l'assegnazione finale si basa sulla matrice C", senza dire come) — qui è implementato seguendo Fred & Jain (2002), la stessa referenza che Zanola cita per la parte "consensus" di RSC.
- **Una differenza rispetto al paper originale**: Fred & Jain tagliano il dendrogramma a una **soglia di similarità `t`** (es. 0.5 — nello stesso cluster solo le coppie co-clusterizzate almeno il 50% delle volte, il numero di cluster finali emerge da solo), non a un numero di cluster fisso. Qui invece si taglia a `n_clusters` esplicito, per coerenza con tutti gli altri metodi della pipeline (che richiedono sempre `n_clusters`/`n_components` in anticipo).
...
- Per scegliere `n_clusters`: **non** si sweeppa `"evidence_accumulation"` direttamente per questo — si sweeppa il `base_method` scelto (es. `"spectral"`) con la diagnostica di stabilità RSC attiva (vedi sezione sotto), che implementa esattamente il criterio di Zanola (massimo spectral gap sulla matrice di co-occorrenza), poi si scrive il valore scelto nella config di `"evidence_accumulation"`.
