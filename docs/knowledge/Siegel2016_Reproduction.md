...
1. **Scelta della parcellazione**: la parcellazione Gordon usata **non include cervelletto e gangli della base** — gli autori stessi notano che includerli potrebbe migliorare i modelli FC-deficit in studi futuri. Rilevante per noi: un atlante combinato a 372 regioni (Glasser+Harvard-Oxford subcorticale) è stato costruito una volta in via esplorativa e includerebbe talamo/caudato/putamen/pallido/ippocampo/amigdala — un potenziale miglioramento strutturale rispetto al paper originale, non solo una replica — ma non è (ancora) la parcellazione adottata in modo definitivo dal progetto.
