...
- **`input_path`**: `(Stringa)` Il percorso della matrice su cui vuoi calcolare i gruppi. DEVE essere una cartella esistente. Se vuoi clusterizzare i dati grezzi, passa il percorso di `data/derived/lesion_matrix/...`. Se vuoi clusterizzare i dati compressi, passa il percorso di `results/dim_reduction/...`.
