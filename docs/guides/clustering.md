...
| **`reduction_params_file`** | *Stringa \| null* | (31-08-26, tag-params-multi-key) Il file `config/registry/params_reduction*.json` che ha prodotto `input_path` — `"config/registry/params_reduction.json"` per lesion, `"config/registry/params_reduction_sdc.json"` per sdc. **Obbligatorio quando `reduced_data: true`**, **vietato (deve mancare) quando `reduced_data: false`**. Serve a leggere `tag_param`/`tag_prefix` del `reduction_method` per ricostruire il tag dell'embedding (es. `"m_euclidean_nc2"`) direttamente dai parametri veri con cui `input_path` è stato costruito — mai scritto a mano in `session_name`, così non può disallinearsi dal vero `input_path` (vedi sotto). |
| **`output_root`** | *Stringa* | Cartella radice per i risultati (es. `"results/lesion/clustering"`). |
| **`session_name`** | *Stringa* | Identificativo di sessione (es. `"s1.1-vol"`). Da quando `reduction_params_file` esiste, **non serve più scriverci a mano l'embedding** (es. `"euclidean_n2"`) — viene derivato automaticamente da `input_path` e aggiunto al nome cartella finale. |
...
**Nome della cartella finale** (31-08-26, tag-params-multi-key): `{dd-mm}_{session_name}_{tag_embedding}_{tag_clustering}` in produzione (es. `31-08_s1.1-vol_m_euclidean_nc2_n_clust5_linkaverage`), `{dd-mm}_{session_name}_{tag_embedding}` in tuning. `tag_embedding` (es. `m_euclidean_nc2`) è derivato automaticamente da `input_path` quando `reduced_data: true` — mai scritto a mano in `session_name` — leggendo `tag_param`/`tag_prefix` del `reduction_method` da `reduction_params_file` e i valori veri usati per `input_path` dal suo stesso `config.md`. `tag_clustering` (es. `n_clust5_linkaverage`) resta il tag già esistente, da `tag_param`/`tag_prefix` di `params_clustering.json`. Nessuno dei due tag è presente quando manca il rispettivo presupposto (`reduced_data: false` → niente `tag_embedding`; metodo senza `tag_param` → niente `tag_clustering`).
...
- `production/<metodo>/runs.csv` per Produzione — colonne `session, reduction_method, reduction_n_components, reduction_metric, <una per ogni chiave di tag_param del metodo>, timestamp, input_path, params, output, notes` (es. per agglomerative: `session, reduction_method, reduction_n_components, reduction_metric, n_clusters, linkage, timestamp, ...`).
- `tuning/<metodo>/runs_tuning.csv` per Fine-Tuning — colonne `session, reduction_method, reduction_n_components, reduction_metric, timestamp, input_path, params, output, notes` (nessuna colonna per singolo hyperparameter del metodo di clustering: un run di tuning sweeppa tutta la griglia, non sceglie un solo valore — ma `reduction_*` restano, descrivono l'embedding sorgente, non il metodo sweeppato).

`reduction_method`/`reduction_n_components`/`reduction_metric` descrivono l'embedding sorgente (`input_path`, quando `reduced_data: true`) — risolti automaticamente leggendo il `config.md` di quel run, mai dichiarati a mano nel config di clustering. Prefisso `reduction_` (non il semplice `n_components`/`metric` che usa `dim_reduction.py`) per non confliggere con l'omonimo hyperparameter proprio del metodo di clustering (es. `gmm` ha già un proprio `n_components`, il numero di componenti della mistura). `reduction_n_components`/`reduction_metric` sono stringa vuota (non assenti) quando `reduced_data: false` o quando il metodo sorgente non ha quel parametro (es. `pca` non ha `metric`). Nessuna colonna `id` (31-08-26, ridondante: nessun lettore la usava, e il tag/embedding sorgente sono già ricostruibili da `output_dir` e dalle colonne sopra). Lo stesso `reduction_method`/`reduction_n_components`/`reduction_metric` compare anche nel `config.md` di ogni singolo run.
