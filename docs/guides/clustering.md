...
`tuning_grid` può dichiarare **uno o due** parametri per metodo (es. oggi: `agglomerative.linkage`, `gmm.covariance_type`, `dbscan.min_samples`, `spectral.n_neighbors`, ciascuno accanto al parametro principale già sweepato). Con un solo parametro il grafico è una curva (una per metrica); con due diventa una heatmap (due assi, una per metrica) — con più di due, `tuning_results.csv` viene comunque scritto con tutte le combinazioni, ma non viene generato nessun grafico (solo un warning in log).
