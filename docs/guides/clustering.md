...
- **Grafici diagnostici**: Agglomerative genera un *dendrogramma* (uno per ogni `linkage` sweeppato, se `"linkage"` è in `tuning_grid`) + una *matrice di distanza interclasse* (una heatmap per ogni `metric` sweeppato, usa la colonna `"dataset"` dei metadati come proxy di ground-truth debole); Spectral genera un *eigengap plot*. HDBSCAN non ne ha uno dedicato.
