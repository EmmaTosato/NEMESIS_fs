...
| **`session_name`** | *Stringa* | Identificativo di sessione (es. `"s1.1"`). Da quando `reduction_params_file` esiste, **non serve più scriverci a mano l'embedding** (es. `"euclidean_n2"`) — viene derivato automaticamente da `input_path` e aggiunto al nome cartella finale. |
...
**Nome della cartella finale** (31-08-26, tag-params-multi-key): `{dd-mm}_{session_name}_{tag_embedding}_{tag_clustering}` in produzione (es. `31-08_s1.1_m_euclidean_nc2_n_clust5_linkaverage`), `{dd-mm}_{session_name}_{tag_embedding}` in tuning. `tag_embedding` (es. `m_euclidean_nc2`) è derivato automaticamente da `input_path` quando `reduced_data: true` — mai scritto a mano in `session_name` — leggendo `tag_param`/`tag_prefix` del `reduction_method` da `reduction_params_file` e i valori veri usati per `input_path` dal suo stesso `config.md`. `tag_clustering` (es. `n_clust5_linkaverage`) resta il tag già esistente, da `tag_param`/`tag_prefix` di `params_clustering.json`. Nessuno dei due tag è presente quando manca il rispettivo presupposto (`reduced_data: false` → niente `tag_embedding`; metodo senza `tag_param` → niente `tag_clustering`).
