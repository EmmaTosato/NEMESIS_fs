...
Confronto **human-in-the-loop** tra 2+ run di clustering già prodotte - nessuna selezione automatica di `k`/metodo (`code_standards.md` §0, stessa filosofia di `clustering.py`/`clustering_tuning.py`), solo tabelle/heatmap da leggere.

- **Notebook**: `notebooks/exploration/clustering_evaluation.ipynb`
...
Locale soltanto, come ogni notebook in `notebooks/exploration/` - non è una pipeline CLI:

```bash
conda activate nemesis
jupyter lab notebooks/exploration/clustering_evaluation.ipynb
```
...
- Nessuna di queste metriche viene letta automaticamente dal notebook per scegliere `k`/metodo - la lettura resta dell'utente, come da `code_standards.md` §0.
