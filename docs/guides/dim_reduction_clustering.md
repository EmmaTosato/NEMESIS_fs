...
- **`regress_out_volume`**: `(Booleano)` Come nella guida al *Dim Reduction*: rimuove per regressione lineare l'effetto del volume lesionale dall'embedding (calcolato una volta sola, prima del clustering) prima di clusterizzarlo. Incompatibile con `metric: "jaccard"`/`"dice"` in `reduction_params_file` (errore esplicito se combinati) — vedi `docs/knowledge/dimensionality_reduction.md`.
