...
**Un soggetto entra nella matrice solo se ha ENTRAMBI**: una lesion mask **registrata** nel TSV clinico del suo dataset (`assets/metadata/<dataset>_participants_lesions.tsv`, colonna `lesion/manual_masks/anat/lesion_mask == "present"`) e il CSV SDC richiesto. Non si controlla più `manual_masks/` su disco direttamente — un retrieval locale parziale renderebbe quel controllo inaffidabile (trovato 27/08/26: su questo Mac solo 10 lesion mask per dataset erano fisicamente presenti, ma il TSV conferma che la quasi totalità della coorte le ha davvero). Un soggetto con output SDC ma senza lesion mask registrata (caso reale confermato in coorte: `sub-STUKLFR0005`, assente dal TSV) viene escluso esplicitamente, non trattato come "disconnessione zero" — vedi `docs/dev/sdc_matrix.md`.
