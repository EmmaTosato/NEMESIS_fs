...
**UMAP** (`results/lesion/dim_reduction/umap/tuning/28-07_s1.1`, griglia `n_neighbors`×`metric`, 15 combinazioni — tabella sotto è una selezione, dati completi in `tuning_results.csv` nella cartella):
...
`jaccard`/`dice` battono `euclidean` di ~0.20 su ogni `n_neighbors` (atteso: normalizzano per il volume lesionale, coerente con `docs/methods/dimensionality_reduction.md`).

**Raccomandazione: n_neighbors=15, metric=jaccard.** Nota: la produzione attuale (`params.umap`: `n_neighbors=5, metric=euclidean`, trustworthiness=0.740 nel `runs.csv`) non usa ancora nessuna delle due — da aggiornare.
