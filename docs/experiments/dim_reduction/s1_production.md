...
##### Obiettivo
- Prima produzione t-SNE valida per s1.1 dopo il tuning (03/04/13-08) 
...
##### Obiettivo
- Riprodurre su s1.2 gli stessi 6 embedding di produzione già fatti su s1.1 (`{dice,euclidean}×{2,3,10}`), stessi `n_neighbors`/`min_dist` per combo 
...
##### Obiettivo
- Prima produzione t-SNE valida per s1.2 dopo tuning del `26-08`
