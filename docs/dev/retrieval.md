...
Everything in this module is organized around a top-level `object`: **`lesion`**, **`feature`**, and **`sdc`** (`KNOWN_OBJECTS = ("lesion", "feature", "sdc")` in `config.py` — a true structural invariant). Everything below it is fully data-driven from the `file_patterns_{local,server}.json` registry, in BIDS-aligned vocabulary:
...
This means `lesion`'s leaves nest **3** levels below `object` (`pipeline → datatype → suffix`, e.g. `lesion.manual_masks.anat.lesion_mask`) while `feature`'s nest **2** (`datatype → suffix`, e.g. `feature.func.FC-pearson`) — depth genuinely varies by object, not a fixed number. `RetrieveItem.path_key()`/`RetrieveItem.from_path()` (`config.py`) are the single place that map between this variable-length tuple and typed fields — nothing else re-derives the shape itself.
...
```json
{
  ...
  "feature": {
    ...
    "func": {
      "FC-pearson": [
        "{subject_id}/func/{subject_id}_space-MNI152NLin6Asym_FC-pearson_atlas-Yan100TianS1Buckner7N.csv",
        "{subject_id}/func/{subject_id}_space-MNI152NLin6Asym_FC-pearson_atlas-Yan100TianS2Buckner7N.csv",
        "... (today 12 templates total: Yan100/200/300/400 × TianS1/S2/S3, all × Buckner7N — see `file_patterns_server.json` for the exact, current list. This set has already changed once since it was first registered and may change again - treat the registry file itself as the source of truth for which atlases exist, never this doc.)"
      ],
      "motion": [
...
Accessors: `.project_root_for(object_)`, `.templates_for(object_, *path)` (raises `ValueError` if unregistered), `.has(object_, *path)`, `.combinations_for(object_)` (every full leaf key under that object, used by `dataset.py`/`verify.py`).
...
2. **Genuinely different, simultaneously-present files** — the current real case for `feature.func.FC-pearson`: today's templates (`Yan{100,200,300,400}TianS{1,2,3}Buckner7N`, 12 as of writing - see `file_patterns_server.json` for the exact current list, since which atlas variants are registered here is expected to change over time) are **not** alternate names for the same atlas — they are different atlases' connectivity matrices that legitimately coexist for the same subject. `resolve()` correctly returns all that exist, and all get copied — that's the desired behavior. Same reasoning applies to `feature.func.motion`/`feature.func.outliers`, each registered as 2 templates (`.tsv` + `.json` sidecar) — not alternates, both wanted every time a subject has either.
...
`Dataset.available(item)` mirrors this dataset-wide: true if *any* subject has a file matching *any* registered template for `item.path_key()`. Used by upfront validation to reject a request a dataset can never satisfy, before any copying starts. A template with `{subject_id}` in more than one path segment (true for every registered `lesion_mask`/`feature` template today - both a directory and the filename) requires every occurrence to bind to the *same* subject, checked via a backreferenced regex (`_subject_id_pattern`) rather than independent glob wildcards - which would happily treat e.g. `sub-A/anat/sub-B_label-lesion_mask.nii.gz` (a real-world mis-copy, folder and filename disagreeing on subject) as "available", even though no real subject would ever produce that path via `resolve()` (`lessons_learned.md` #22).

### Subject-group naming (`group_of`, `KNOWN_SITES`)

`group_of(subject_id) -> str` (module-level, not just `Dataset.group_of` - so any layer that discovers subjects by globbing directly, e.g. `src/features/functional.py`, which doesn't go through `Dataset`, can still tell a healthy control from a patient without duplicating the regex) parses `sub-<DISEASE><SITE>[HC]<NUM>` into one of `'ST'|'HC'|'PD'|'GM'`. `KNOWN_SITES = ("UNIPD", "UKLFR", "UCLUK", "UKE")` is a deliberately closed, explicit list, not a free-form `[A-Z]+?` character class: a lazy regex can't tell "site code that happens to end in HC" apart from "site code + HC (healthy control) marker" - a hypothetical future site `"MONTREALHC"` would always mis-split into `site="MONTREAL"` + `hc=True`, silently misclassifying every stroke patient from that site as a healthy control (`lessons_learned.md` #20). Adding a new site requires adding it to `KNOWN_SITES` explicitly - an unregistered site raises `ValueError`, never guessed.
...
A subject who only exists under some *other* object/pipeline not requested at all (e.g. only has a `feature` file, when this run only requests `lesion`) is not a member of this run at all: not selected, not counted in `subjects_selected`, and — the point of this design — **not reported as "File not found" either**. `copy_summary` only ever explains gaps for exactly what was asked for; it deliberately makes no attempt to answer "what does this dataset have everywhere" — that's out of scope for this report.
...
Known groups: `ST`, `HC`, `PD`, `GM`. Known objects: `lesion`, `feature`, `sdc` (`KNOWN_OBJECTS`). Whether `pipeline` is required or forbidden per object: `_OBJECTS_REQUIRING_PIPELINE`/`_OBJECTS_FORBIDDING_PIPELINE`. Known `datatype`/`suffix` values: whatever is registered in `file_patterns.json` for a given object — not a static list in code.
...
Written to `summaries/data_retrieval/<project>/copy_summary__<dd-mm-yy>__<hh-mm-ss>.md` (the timestamp includes seconds so two runs started within the same minute - e.g. a quick resubmit after a transient mount blip - never collide and silently overwrite each other's report). Title is `<project>_<dd-mm-yy>` with the run time as a subtitle.

This report only explains what **this run** did and did or didn't find for the exact combinations in its `retrieve` list — it deliberately does not try to answer "what does this dataset have in general". Content, in order:
...
A `File not found` line (`"<dataset>: <subject_id> - no <path_key joined by '/'> (<reason>)"`) does **not** say anything about whether the subject has data under some other retrieve item — this report is scoped to exactly the `retrieve` list it was given, never a cross-modality comparison.
...
**Resolved since this was written**: this section used to flag two test gaps in `test_resolution_counts.py` — `FILE_PATTERNS_PATH` hardcoded to `config/registry/file_patterns.json`, a filename that no longer existed, and the FC-pearson tests hardcoding an outdated 2-atlas case. Both are fixed today: `FILE_PATTERNS_PATH` points at `file_patterns_server.json`, and `test_subject_with_every_fc_pearson_atlas_gets_every_file` derives its expected templates dynamically from the registry itself (`FILE_PATTERNS.templates_for("feature", "func", "FC-pearson")`) rather than hardcoding any atlas name or count — so it stays a meaningful check even if the registered atlas set changes again (as it already has once, see the note on `FC-pearson` templates above).
...
This refactor deliberately covers only `lesion`/`manual_masks` (derivatives) and `feature`/`{FC-pearson,motion,outliers}` — for `FC-pearson`, a curated subset of the atlas templates available on disk, not the full ~30 variants. At the time this section was first written, that subset was `GlasserTianS{1,2,3}Buckner7N` + `Schaefer{100,200,300,400}TianS{1,2,3}Buckner7N` (15 templates, chosen for consistency with the Glasser-based combined atlas already built for lesions, plus multi-resolution Schaefer). **That choice has since changed**: the registry today (`file_patterns_server.json`) registers a different, Yan-based subset instead (`Yan{100,200,300,400}TianS{1,2,3}Buckner7N`, 12 templates) — treat the registry file itself, never this paragraph, as the source of truth for which atlases are registered at any given moment, since the set has already changed once and is expected to keep evolving (atlases added or dropped) as the project's needs change. `AAL116`, `Brainnetome246`, `DesikanKilliany[Tourville]Aseg` remain on disk but unregistered, and can be added the same way if a future task needs them — **native/raw BIDS data is fully descoped**, for concrete reasons found while surveying the real EBRAIN mount:
...
**`sdc` — a concrete instance of this extension (added 25-08-26)**: Stage1+Stage2 BCBToolKit output - the same computation `src/sdc/`/`compute_sdc.py` itself performs - mirrored on EBRAIN under `Clinical_connectome/features/Clinical_connectome_stroke/<dataset>/lesion/{subject_id}/...` for `UNIPD/WashU`, `UNIPD/PASPORT`, `UNIPD/PSP`, `UKLFR/stroke_UKLFR`, and a 6th dataset new to this registry, `UKE/WAKEUP_acute` (site code `UKE` added to `dataset.KNOWN_SITES`). This particular run was not orchestrated through our own `compute_sdc.py --mode manifest/run/aggregate` CLI, so it has no `manifest.csv`/`_status/*.json`/`runs.csv` row written by us - the only real difference from a `compute_sdc.py` production run is provenance tracking, not the computation itself. Registered like `feature` (`_OBJECTS_FORBIDDING_PIPELINE` - no discoverable pipeline name at the source either), with `datatype="dwi"` and 6 suffixes (`disconnectome-map`/`disconnectome-mapstats`/`disconnectome-LF`, `lesion-map`/`lesion-mapstats`/`lesion-LF` - the resampled `res-1` lesion mask alongside the disconnectome itself; `LF` suffixes each carry 15-16 per-atlas CSV templates, same "grab every one that exists" pattern as `feature.func.FC-pearson`'s atlas list). Deliberately its own `object`, not folded into `feature` despite sharing that object's `project_root`'s parent tree: `Dataset._subject_container()` picks **one** representative template per `(object, pipeline)` to derive where subject folders live (see above) - `sdc`'s templates all start with `lesion/{subject_id}/...` while `feature.func.FC-pearson` starts with `{subject_id}/...` directly, and both share `pipeline=None`, so mixing them under one object would make `_subject_container("feature", None)` pick whichever leaf sorts first alphabetically, silently breaking subject discovery for the other (see `combinations_for()`'s alphabetical sort). Retrieved via its own config, `config/pipelines/retrieval_sdc.json` (`group_filter: ["ST"]`, `include_tabular_data: false` - `participants_tsv_path()` always resolves through the `lesion` object's root regardless of what's requested, which `UKE/WAKEUP_acute` doesn't have), landing locally under `sdc` (`output_layout._LOCAL_PIPELINE_LABEL_FOR_OBJECT`) - same label as the object itself, since this is genuine Stage1+Stage2 output, just not run via our own orchestration. Landed locally as `sdc/<subject_id>/<filename>`, no `dwi/` folder (`output_layout._OBJECTS_WITHOUT_DATATYPE_FOLDER`, added 04/09 - the local layout originally mirrored `datatype` as a folder for every object, matching `anat`/`func`'s real source folders, but `dwi` was never a real folder for `sdc` at the source either, so the fabricated local `dwi/` nesting was removed; the ~1600 already-retrieved subjects across all 5 datasets were migrated in place, one directory level up, rather than re-retrieved).

**`UKE/WAKEUP_acute`'s `manual_masks` (local-only, added 03/09)**: `UKE` has no `lesion` `project_root` on the server at all - not a gap in what we've retrieved, a genuine absence at the source, so no `retrieve_data.py` run can ever populate it. What exists locally today (`derivatives/UKE/WAKEUP_acute/manual_masks/{subject_id}/anat/{subject_id}_space-MNI152NLin6Asym_label-lesion_mask.nii.gz`, all 451 subjects) is a **manual, one-off promotion** of `sdc`'s own `lesion-map` leaf (the lesion mask BCBToolKit resampled as its own computation input, `res-1` in the filename) - copied out, renamed to match the `lesion.manual_masks.anat.lesion_mask` template exactly (dropping `res-1`, which `Dataset.resolve()` requires char-for-char, not fuzzy-matched), then deleted from its original `sdc/{subject_id}/` location so it isn't duplicated on disk. Two consequences worth knowing before touching either object for UKE again: `sdc.lesion-map` now resolves to nothing for every UKE subject (deliberate, not a retrieval gap), and nothing in `src/retrieval/` did this - it's not reproducible by re-running any pipeline, only by redoing the same manual steps against a fresh `sdc` pull.
