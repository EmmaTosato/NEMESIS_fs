...
Can also run **locally against the Mac's own `data/`** (checks `file_patterns_local.json`'s project roots, not the EBRAIN source) via a dedicated config, `config/pipelines/data_summary_local.json` — kept separate from `retrieval_local.json` on purpose: that config's `datasets`/`group_filter` scope a real copy run, while this one just wants every dataset with local data and no group filtering (`group_filter: null`), which would be the wrong default for an actual retrieval. `file_patterns_local.json` registers `sdc` (added 03/09, mirroring the server's `sdc.dwi.*` leaves under `sdc/{subject_id}/dwi/`, the layout `retrieve_data.py`/`download_sdc.py` write to locally) alongside the pre-existing `lesion`/`feature`:

```bash
PYTHONPATH=. conda run -n nemesis python scripts/data_summary.py --config config/pipelines/data_summary_local.json
```
