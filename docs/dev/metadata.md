...
data/clinical_connectome/derivatives/<dataset>/participants.tsv   (raw, per dataset)
        │  notebooks/exploration/dataset_exploration.ipynb
        ▼
...
**Not produced by any `src/pipeline/` script** — generated by `notebooks/exploration/dataset_exploration.ipynb`, which intersects `assets/dataset_summaries/data_summary__*.csv` (retrieval availability flags) with each dataset's own raw `participants.tsv`. Re-run the relevant notebook cells to regenerate; nothing in `src/` writes here.
