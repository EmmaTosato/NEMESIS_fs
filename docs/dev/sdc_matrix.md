...
Fixed by resolving lesion-mask presence from `assets/metadata/participants.csv` instead - the authoritative registry of which subjects have a lesion mask at all, independent of what's currently retrieved on any one machine. A genuine `excluded_no_lesion_mask` case is a subject whose registry row has `has_lesion == False`, or which has no registry row at all (verified real example: `sub-STUKLFR0005`). `_subjects_with_lesion_mask` raises `ValueError` if a requested dataset has no row whatsoever in the registry - a structural gap this pipeline's core admission criterion depends on, never silently treated as "nobody in that dataset has a lesion mask".

A full local retrieval run before this fix admitted 40/1151 subjects; after the fix, the same local run (SDC CSVs untouched, only the admission criterion changed) admits 1119/1151 - the remaining 32 exclusions are all genuine registry gaps (subject not flagged `has_lesion` in the registry), confirmed individually, not local-retrieval artifacts.
