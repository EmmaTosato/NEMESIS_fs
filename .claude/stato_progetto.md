...
## Sessione 2026-07-25 — Atlante SDC completo, fix crash 100% dei soggetti, output appiattito

**Obiettivo**: passare la pipeline `compute_sdc` dall'atlante trattografico bundled parziale (10 soggetti) a quello completo (178 soggetti HCP 7T), lanciarla in produzione su SLURM, e sistemare tutto quello che emergeva nel farlo. Sessione svolta in parte in parallelo con un'altra sessione dello stesso utente (commit `56be3e5`, `0f24090` non fatti da questo agente ma convergenti sullo stesso lavoro — vedi sotto).

**Decisioni prese / concetti discussi**:
- **Atlante trattografico**: `tracks7T.zip` (125GB, 178 file `.trk` HCP 7T 1mm) trovato già caricato in `/data/corbetta/Clinical_connectome/BCBToolkit/TRK/tracks7T.zip`, integrità verificata (`unzip -t`, 181/181 file OK), estratto in loco sullo stesso mount (niente copia cross-filesystem) in `/data/corbetta/Clinical_connectome/BCBToolkit/TRK/tracks7T/`. `config/pipelines/compute_sdc.json` → `tracks_dir` punta lì. Perché 178 e non un sottoinsieme più piccolo: il paper di riferimento del metodo (Thiebaut de Schotten et al. 2020) usa 163 controlli sani come campione primario; un campione di soli 10 (testato come verifica di robustezza nel paper) dà r=0.866 di correlazione col campione pieno — non equivalente, solo "buono".
- **`MaxArraySize=1001`**: il cluster rifiuta un array SLURM da 1150 task (1 task/soggetto, il default della doc). `jobs/run_compute_sdc.sh` chunkato a 575 task × 2 soggetti/task (stride slice, copertura completa), throttle `%3` per rispettare la quota utente di 3 job concorrenti massimi (non negoziabile, confermato dall'utente). Tempo stimato per il run completo con questi vincoli: ~6 giorni (575 task ÷ 3 concorrenti × ~45min/task misurati su un run reale).
- **Bug critico trovato e risolto — root cause del 100% di fallimento al primo run reale**: `bcblib` (pacchetto pip di terze parti, non nostro) rileva la risoluzione della lesion mask da una whitelist fissa di shape 1mm/2mm; il dataset `UKLFR/stroke_UKLFR` (697/1150 soggetti, 61% del manifest) è nativamente a 1.5mm → crash immediato. Amplificato dal chunking a 2 soggetti/task: `bcb-lf-preprocess` abortisce l'intero batch al primo errore, quindi anche i soggetti UNIPD sani accoppiati nello stesso task venivano marcati falliti. Narrativa completa in `docs/debugging/debug_23_07_26.md`, pattern generale in `.claude/lessons_learned.md` #13. **Fix**: nuovo modulo `src/sdc/resample.py` (`resample_lesion_if_needed`/`resample_nonconforming_rows`) — ricampiona su griglia 1mm canonica (nearest-neighbour) qualunque lesion mask non già conforme, PRIMA di passarla a `bcb-lf-preprocess`, così la sua stessa whitelist la riconosce. Verificato end-to-end sul soggetto reale che crashava (`sub-STUKLFR0128`) sia isolatamente sia dentro la pipeline completa (Stage1+Stage2 reali, non dry-run).
- **Output finale riorganizzato due volte nella stessa sessione**: (1) Stage1 (NIfTI) e Stage2 (CSV/TSV) inizialmente scrivevano in `prep/`/`features/` separati sotto la sessione — cambiato per scrivere entrambi nella stessa cartella `sub-<id>/lesion/` per soggetto (Stage2 non tocca/cancella l'input di Stage1, quindi puntare `--output-dir` di `bcb-lesion-features` sullo stesso `validated_prep_dir` usato come `--prep-dir` li fa convivere senza simlink custom). (2) Su richiesta esplicita dell'utente, la sottocartella `lesion/` (nome interno di `bcblib`, non un vero datatype BIDS, ridondante perché sempre presente una sola volta per soggetto) è stata rimossa dall'output finale: l'aggregate ora simlinka `output_dir/<subject>/` direttamente a `.../prep/<subject>/lesion/` invece che a `.../prep/<subject>/`. Assunzione implicita: nessuna sessione (`ses-*`) nello staging — vero oggi, da rivalutare se in futuro serviranno sessioni multiple per soggetto.
- **Bug minore trovato e corretto in `scripts/run_compute_sdc_no_slurm.sh`**: leggeva una chiave di config inesistente `run_name` invece di `session_name` (avrebbe scritto i log in una cartella `null/`); e non vincolava `OPENBLAS_NUM_THREADS`/`OMP_NUM_THREADS`, causando oversubscription di thread quando il pool locale lancia più processi `bcb-lf-preprocess` in parallelo sulla stessa macchina (ogni processo prova a usare tutti i core host di default) — 7/8 task locali fallivano con `pthread_create failed`. Non impatta la produzione SLURM (isolamento per task via cgroup).

**File modificati/creati** (verificato con `git log --oneline 989f288..HEAD` e `git show --stat`):
- Codice: `src/sdc/resample.py` (nuovo), `src/sdc/config.py` (+`mni152_reference_path`, validato al load), `src/sdc/status.py` (+stato `failed_resample`), `src/pipeline/compute_sdc.py` (resample hook, merge Stage1+Stage2, appiattimento `lesion/`), `src/sdc/runner.py` (`run_stage2` non prende più `features_dir`)
- Test: `tests/unit/test_sdc_resample.py` (nuovo, 6 test), `tests/unit/test_sdc_config.py`/`test_sdc_manifest.py` (aggiornati per `mni152_reference_path`)
- Config/job: `config/pipelines/compute_sdc.json` (`tracks_dir`), `jobs/run_compute_sdc.sh` (array 575×%3, commenti aggiornati), `scripts/run_compute_sdc_no_slurm.sh` (fix `run_name`, `OPENBLAS_NUM_THREADS`)
- Doc: `docs/guides/compute_sdc.md` (nuovo layout output, resampling), `docs/debugging/debug_23_07_26.md` (nuovo), `.claude/lessons_learned.md` (pattern #13)

**Stato dei test**: `pytest tests/unit/ -k sdc -q` → **35/35 passano** (rilanciato più volte durante la sessione, ultima volta dopo il fix di appiattimento). Suite completa (`pytest tests/ -q`) **non rilanciata** in questa sessione — solo il sottoinsieme SDC toccato.

**Verifica end-to-end**: la coppia `sub-STUNIPD0001` (UNIPD, 2mm, prima falliva per associazione) + `sub-STUKLFR0128` (UKLFR, 1.5mm, root cause) completa Stage1+Stage2 con successo dopo ogni fix, ripetuto 3 volte in questa sessione (fix resampling, fix merge output, fix appiattimento) — ogni volta con calcolo reale (non `--dry-run`), ~45-50 min per run.

**Prossimo passo esatto**: `data/derived/sdc/s1/` è stata svuotata dall'utente a fine sessione (manifest + scratch di test, non l'array reale — mai stato rilanciato dopo l'ultimo fix). Per riprendere: rigenerare il manifest (`python -m src.pipeline.compute_sdc --config config/pipelines/compute_sdc.json --mode manifest`, atteso 1150 soggetti/0 esclusi) e sottomettere l'array reale su SLURM:
```bash
ARRAY_JOBID=$(sbatch --parsable jobs/run_compute_sdc.sh)
sbatch --parsable --dependency=afterany:${ARRAY_JOBID} jobs/run_compute_sdc_aggregate.sh
```
Nessun altro fix noto in sospeso — tutti i bug trovati in questa sessione sono risolti e verificati. Monitorare con `squeue -u etosato` e il conteggio per-stato in `data/derived/sdc/s1/_status/*.json` (vedi comandi in `docs/guides/compute_sdc.md`). Stima ~6 giorni per il completamento con la quota di 3 job concorrenti.
