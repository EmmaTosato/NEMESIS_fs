...
- `docs/` — stable, incremental project documentation (distinct from raw meeting notes, which live under `assets/`):
  ...
  - `docs/guides/` — user-facing "how do I run this" guides, one per pipeline (`retrieval.md`, `atlas_building.md`, `matrix_building.md`, `fc_matrix_building.md`, `dim_reduction.md`, `clustering.md`, `compute_sdc.md`, `embedding_app.md`, `understanding_umap_report.md` — `dim_reduction_clustering.md` retired 14-08-26 along with the one-shot CLI it documented, see `docs/dev/clustering_migration_plan.md`) plus `datasets.md` (EBRAIN paths, per-dataset N, per-subject folder structure, `participants.tsv` conventions)
