...
> **Meccanica dettagliata UMAP/t-SNE** (letteratura, come interpretarli): `umap_tsne_guide.md`.
> **Come leggere gli indici di tuning** (trustworthiness, varianza spiegata): `knowledge/dim_reduction_clustering/dim_reduction_tuning_guide.md`.
> **Rassegna più ampia di metodi** (anche non usati nel progetto): `knowledge/dim_reduction_clustering/dim_reduction_literature_survey.md`.
...
Guida in linguaggio semplice a cosa fa ciascun metodo di riduzione dimensionale **scelto** nel progetto — PCA, t-SNE, UMAP. Non un manuale di ML, solo il minimo per capire cosa controlla un iperparametro in config e con qualche criterio per scegliere tra i metodi. `src/analysis/reduction.py` implementa anche PaCMAP e PCA con rotazione varimax: non trattati qui perché non fanno parte del set di metodi che usiamo attivamente — la loro meccanica/gotcha di config restano documentati nel codice stesso e in `knowledge/dim_reduction_clustering/dim_reduction_literature_survey.md`.
...
(`metric` è organizzato come "parametro annidato" nel tuning — una sottocartella per valore invece di una griglia unica con `perplexity`; vedi `knowledge/dim_reduction_clustering/dim_reduction_tuning_guide.md`.)
...
- **La presunta superiorità di UMAP nel preservare la struttura globale è ridimensionata in letteratura**: gran parte del vantaggio percepito viene dalla sua inizializzazione di default (basata su spectral embedding), non dalla funzione di costo in sé (Kobak & Linderman 2021) — vedi `umap_tsne_guide.md` per i dettagli.

### UMAP e clustering: quante componenti?

**Nessun numero fisso** — verificato su più fonti (2026-08 review, approfondito ulteriormente in `knowledge/dim_reduction_clustering/challenge_of_clustering_after_dim_reduction.md`), nessuna prescrive un default valido in generale:
...
- Una critica indipendente (blog, non peer-reviewed) solleva la stessa preoccupazione con più forza ("self-affirming bias": l'embedding crea la struttura che poi il clustering "conferma") — non aggiunge autorità, ma la sua tesi centrale coincide con l'avvertimento sopra, che viene dagli stessi autori di UMAP. Discussione completa (incluso il contro-argomento empirico su MNIST che ridimensiona la tesi scettica) in `knowledge/dim_reduction_clustering/challenge_of_clustering_after_dim_reduction.md`.
