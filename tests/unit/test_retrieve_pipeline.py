...
def test_explicit_subject_absent_from_one_dataset_reported_as_missing(tmp_path):
    ...
    assert "UNIPD/WashU: sub-STUNIPD0500 - not present in this dataset" in stats["UNIPD/WashU"].missing
    assert "UNIPD/PASPORT: sub-STUNIPD0001 - not present in this dataset" in stats["UNIPD/PASPORT"].missing
...
def test_retrieve_subject_flags_ambiguous_match(tmp_path):
    ...
    assert "sub-STUNIPD0001" in stats["UNIPD/WashU"].ambiguous[0]
...
def test_build_report_includes_ambiguous_and_non_conforming_sections(tmp_path):
    ...
    stats = {
        "UNIPD/WashU": retrieve_data.DatasetStats(
            ambiguous=["UNIPD/WashU: sub-A - native/lesion_roi: using X, also matched Y"],
