...
def test_verify_dataset_flags_checksum_mismatch(tmp_path):
    ...
    project_root, ds = _make_dataset(tmp_path)
    config = _make_config(tmp_path, project_root)
    _touch(_local_lesion_mask_path(config, "sub-STUNIPD0001"), b"stale-content")

    result = verify.verify_dataset("UNIPD/WashU", ds, ["sub-STUNIPD0001"], config)

    assert len(result.mismatched) == 1
    assert "sub-STUNIPD0001" in result.mismatched[0]
    assert "lesion/manual_masks/anat/lesion_mask" in result.mismatched[0]
...
def test_verify_dataset_skips_subjects_where_source_has_no_file(tmp_path):
    ...
    phase."""
    project_root, ds = _make_dataset(tmp_path)
    _touch(project_root / "UNIPD" / "WashU" / "derivatives" / "manual_masks" / "sub-STUNIPD0002" / "anat" / "placeholder.txt")
    config = _make_config(tmp_path, project_root)
...
def test_verify_dataset_skips_retrieve_item_whose_object_this_dataset_lacks(tmp_path):
    ...
    target - it was never expected to land locally either (see
    retrieve_data._report_skipped_retrieve_items)."""
    project_root, ds = _make_dataset(tmp_path)
    file_patterns = _make_patterns(project_root)
