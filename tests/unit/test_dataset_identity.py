...
def _make_patterns(project_root):
    """Minimal registry: one object (lesion), one pipeline (manual_masks),
    one leaf (anat/lesion_mask) - enough for subjects()/
    non_conforming_subject_folders() to derive where subject folders live
    (they no longer assume a fixed root; see Dataset._subject_container)."""
    return FilePatterns(
        project_roots={"lesion": project_root},
        patterns={
            ("lesion", "manual_masks", "anat", "lesion_mask"): [
                "derivatives/manual_masks/{subject_id}/anat/{subject_id}_lesion_mask.nii.gz"
            ]
        },
    )
...
def test_missing_root_raises(tmp_path):
    ...
    with pytest.raises(FileNotFoundError):
        ds.subjects("lesion", "manual_masks")
...
def test_subjects_lists_all_sorted(tmp_path):
    ...
    ds = Dataset("UNIPD/WashU", _make_patterns(project_root))
    assert ds.subjects("lesion", "manual_masks") == ["sub-STUNIPD0001", "sub-STUNIPD0003", "sub-STUNIPDHC0002"]
...
def test_subjects_filtered_by_group(tmp_path):
    ...
    ds = Dataset("UNIPD/WashU", _make_patterns(project_root))
    assert ds.subjects("lesion", "manual_masks", group="ST") == ["sub-STUNIPD0001"]
    assert ds.subjects("lesion", "manual_masks", group="HC") == ["sub-STUNIPDHC0002"]
    assert ds.subjects("lesion", "manual_masks", group="PD") == []
...
def test_subjects_excludes_non_conforming_folders(tmp_path):
    ...
    ds = Dataset("UNIPD/WashU", _make_patterns(project_root))
    assert ds.subjects("lesion", "manual_masks") == ["sub-STUNIPD0001"]
...
def test_non_conforming_subject_folders_lists_them(tmp_path):
    ...
    ds = Dataset("UNIPD/WashU", _make_patterns(project_root))
    assert ds.non_conforming_subject_folders("lesion", "manual_masks") == ["sub-TEST", "sub-backup_old"]
...
def test_non_conforming_subject_folders_empty_when_all_conform(tmp_path):
    ...
    ds = Dataset("UNIPD/WashU", _make_patterns(project_root))
    assert ds.non_conforming_subject_folders("lesion", "manual_masks") == []
